...
int main()
{
  ...
  h.onMessage([&steer_pid, &throttle_pid](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
	  ...
	  // Simple croipping of the feedback works slightly better than the logistic function
	  // (it introduces less gidder)
	  if (steer_y < -1 ) steer_value = -1;
	      else if (steer_y > 1 ) steer_value = 1;
		else steer_value = steer_y;
	  
	  /*if (throttle_y < 0 ) throttle_value = 0.1;
	      else if (throttle_y > 1 ) throttle_value = 1;
		else throttle_value = throttle_y + .1;
	  */
